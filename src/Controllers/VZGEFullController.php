...
class VZGEFullController extends Controller
{
    // 全身图尺寸
    const FULL_SIZE = 512;
    ...
    // 第三方API基础URL
    const API_BASE_URL = 'https://visage.surgeplay.com/full/';
    
    /**
     * 通过UUID或玩家名获取全身图
     */
    public function FullByUuid(Request $request, $identifier)
    {
        // 尝试作为UUID查询
        ...
        if ($uuidRecord) {
            ...
            if ($player) {
                return $this->getFullBody($player);
            }
        }
        ...
        // 两种方式都找不到玩家，使用默认皮肤
        return $this->generateDefaultFullBody();
    }
    
    /**
     * 获取玩家全身图
     */
    protected function getFullBody(Player $player)
    {
        // 如果玩家有皮肤纹理，使用本地皮肤文件
        if ($player->tid_skin) {
            ...
            // 使用本地皮肤生成全身图
            return $this->generateFullBodyBySkin($skinContent, $cacheKey);
        }
     ...
     * 使用皮肤内容生成全身图
     */
    protected function generateFullBodyBySkin($skinContent, $cacheKey)
    {
        // 将皮肤内容转换为base64（去除数据URI前缀）
        $base64Skin = base64_encode($skinContent);
        
        // 构建API URL
        $apiUrl = self::API_BASE_URL . self::FULL_SIZE . '/' . $base64Skin;
        ...
        try {
            ...
            if ($response->getStatusCode() !== 200) {
                abort(500, 'Failed to generate full body image by skin');
            }
     ...
     * 生成默认全身图
     */
    protected function generateDefaultFullBody()
    {
        // 默认皮肤名称
        $defaultSkinName = 'X-Steve';
        
        // 生成缓存键（基于默认皮肤名称）
        $cacheKey = 'full_default_' . md5($defaultSkinName);
        ...
        // 构建API URL
        $apiUrl = self::API_BASE_URL . self::FULL_SIZE . '/' . urlencode($defaultSkinName);
        ...
        try {
            ...
            if ($response->getStatusCode() !== 200) {
                abort(500, 'Failed to generate default full body image');
            }
